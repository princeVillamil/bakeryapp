...
//     .map((id) => parseInt(id.replace("BKR-", "")))
//     .filter((n) => !isNaN(n));
...
export default function AddStock({ onSave, existingIds = [] }) {
  ...
  const derivedStatus = getStatus(form.stock);
  ...
  const validate = () => {
    ...
    if (!form.sellPrice || isNaN(Number(form.sellPrice)) || Number(form.sellPrice) <= 0)
      e.sellPrice = "Enter a valid sell price.";
  ...
  const handleSubmit = async () => {
    ...
    const newItem = {
      ...
      expiry: new Date(form.expiry).toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      }),
    ...
    await addInventory(newItem); // <-- API call
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm text-gray-400 mb-6">
            <span className="font-mono text-gray-500">{newId}</span> has been added to inventory.
          </p>
          <NavLink
          ...
          >
            <button
            onClick={handleReset}
            className="flex items-center gap-3 px-3 py-2 mx-2 my-1 text-sm rounded-xl border border-gray-200 text-gray-500 
          ...
          {/* <button
            onClick={handleReset}
            className="px-5 py-2 text-sm font-medium bg-black text-white rounded-lg hover:bg-gray-800 transition-colors"
          >
            + Add Another Item
          </button> */}
        </div>
      ...
      <div className="container p-4 mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Add a new baked good to the inventory
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 p-3 bg-gray-50 rounded-xl border border-gray-200">
          ...
          <div className="ml-auto">
            <span className={`px-2.5 py-1 font-semibold rounded-full text-[11px] ${STATUS_STYLES[derivedStatus]}`}>
              {derivedStatus}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">
              Stock Quantity <span className="text-black">*</span>
            </label>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                placeholder="0"
                className={`w-32 px-3 py-2 text-sm text-black border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all ${
              ...
              {form.stock !== "" && (
                <span className={`ml-auto px-2.5 py-1 font-semibold rounded-full text-[11px] ${STATUS_STYLES[derivedStatus]}`}>
                  {derivedStatus}
                </span>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
            <label className="block text-xs text-gray-400 uppercase tracking-wider mb-3">
              Pricing <span className="text-black">*</span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="0.00"
                  className={`w-full px-3 py-2 text-sm text-black border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all ${
                    errors.costPrice ? "border-red-300 bg-red-50" : "border-gray-200"
                  }`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="0.00"
                  className={`w-full px-3 py-2 text-sm text-black border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all ${
                    errors.sellPrice ? "border-red-300 bg-red-50" : "border-gray-200"
                  }`}
                />
            ...
            {margin !== null && (
              <div className="mt-3 flex items-center gap-2">
                ...
                <p className="text-xs text-gray-500 shrink-0">
                  <span className="font-semibold text-gray-800">{margin}%</span> margin
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 pt-2 pb-8">
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-6 py-2 text-sm font-semibold bg-black text-white rounded-lg hover:bg-gray-800 active:scale-95 transition-all"
            >
              Add to Inventory →
            </button>
          </div>
