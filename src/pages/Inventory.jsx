...
function ItemForm({ initial, onSave, onCancel, isEdit = false }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <PillSelector label="Category" field="category" options={["Breads", "Pastries", "Muffins", "Cakes", "Cookies"]} />
        ...
        <Modal title="Item Details" onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div className="flex items-start justify-between">
              ...
              <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${STATUS_STYLES[modal.item.status]}`}>
                {modal.item.status}
              </span>
            </div>
